...
code-to-skill 从两个来源提取 SkillAtom：**代码仓库**（M1）和 **知识文档**（M2）。代码仓库准备好 `external/fineract-develop` 即可（见 README），下面对知识文档做详细说明。
...
```bash
# 从目标仓库直接复制
cp external/fineract-develop/README.md kb/fineract/
cp external/fineract-develop/CONTRIBUTING.md kb/fineract/
...
sources:
  docs:
    - id: repo-readme
      path: external/fineract-develop/README.md
      provider: local_file
      type: markdown
```
