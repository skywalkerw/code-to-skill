...
| 优先级 | 改进 | 预估工时 | 对当前项目的影响 |
|--------|------|----------|-----------------|
| P0 | 预编译 tree-sitter grammar → vendor/ | 2h | M3 抽取质量大幅提升 |
| P0 | 图遍历 API（callees/callers） | 1h | M3 可追踪调用链 |
| P1 | SQLite 持久化 | 3h | 增量更新基础 |
| P1 | 符号搜索接口 | 1h | M3/M4 可按需查询 |
| P2 | 框架提取器 | 各 2h | 覆盖 Spring/Vue 等框架 |
