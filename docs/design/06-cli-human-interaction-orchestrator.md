...
# === 项目基础 ===
project:
  name: payment-agent-skill
  domain: payment
  description: "支付系统 Agent Skill 优化项目"

# === 数据源 ===
sources:
  repos:
    - id: payment-service
      path: /abs/path/payment-service
      ref: abc123  # commit SHA 或 branch
      include: ["src/**"]
...
{
  "schema_version": "1.0",
  "run_id": "payment-skill-20260603-001",
  "domain": "payment",
...
```json
{
  ...
  "decision": "approved",
  "scope": "run_id=payment-skill-20260603-001",
  "ts": "2026-06-03T00:00:00Z"
}
```
...
```bash
skill-lab init --workspace ./agent-skill-lab --domain payment
```
...
```bash
skill-lab status payment-skill-20260603-001
```
...
```bash
skill-lab publish payment-skill-20260603-001 --target ~/.codex/skills/payment-agent
```
...
```bash
skill-lab resume payment-skill-20260603-001
```
...
? SkillOpt target is `codex-cli-target` with workspace write enabled.
  Workspace: /abs/path/payment-service
  Approve? [y/N]
```
